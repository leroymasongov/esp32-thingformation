...
static int cntg = 0; // memory of the button press

static void IRAM_ATTR gpio_isr_handler(void* arg)
{
    uint32_t gpio_num = (uint32_t) arg;
    // printf("Interrupt triggered on %d\n",(int)gpio_num);
...
void app_main(void)
{
    ...
    int iteratedInterval = 0;
    while (1) {
        printf("cnt: %d\n", cnt++);
        // each button press to increase frequency of light blinking by cnt = cnt + cntg
        iteratedInterval = baseInterval + cntg;
        printf("iteratedInterval: %d\n", iteratedInterval); 
        vTaskDelay(iteratedInterval  / portTICK_PERIOD_MS); 
        gpio_set_level(GPIO_OUTPUT_IO_0, cnt % 2); // mod 2 is blinking
        gpio_set_level(GPIO_OUTPUT_IO_1, cnt % 2); // maybe add a new led circuit and play with mod 3?
    }
}
